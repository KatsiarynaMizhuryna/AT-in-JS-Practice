...
Then('I expect to see error messages {string} and {string}', async function (emailError, passwordError) {
...
Then('I should see the error message {string} for email', async function (expectedErrorMessage) {
    await LoginPage.emailError.waitForDisplayed({ timeout: 3000 });
...
Then('I should see the login error message {string}', async function (expectedErrorMessage) {
...
Then('I should be redirected to the power tools category page', async function () {
    await browser.waitUntil(async () => {
...
Then('I should see {string} in the language button', async function (expectedLanguage) {
    const languageButton = HomePage.languageButton;
